...
import { ITEM_LABELS, ItemType, PROCESSING_RATES } from '../models/item.js';
import {
  buildItems,
  getAndClearDeposits,
  getHistory,
...
async function handleDeposit(machineTag: string) {
  ...
  const { processed, totalValue } = await processItems(allItems, (item, index, total) => {
    saveDeposit(item);
    console.log(
...
async function handleRefund(machineTag: string) {
  ...
  if (!confirm) {
    ...
    log(
      machineTag,
      'REFUND_CANCELLED',
      `Refund cancelled for ${items.length} item(s), total ${total / 100} kr`
    );
    return;
  }
  ...
  log(
    machineTag,
    'REFUND_ISSUED',
    `Issued refund for ${items.length} item(s), total ${total / 100} kr`
  );
}
...
async function handleHistory(machineTag: string) {
  ...
  history.forEach((d) =>
    console.log(
      `  [${new Date(d.createdAt).toLocaleString('nb-NO')}] ${d.event} - ${d.details || ''}`
    )
  );
}
